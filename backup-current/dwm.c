...
drawbar(Monitor *m)
{
	...
	drw_setscheme(drw, scheme[SchemeNorm]);
	drw_rect(drw, m->ww - (int)(m->ww * 0.5), 0, (int)(m->ww * 0.5), bh, 1, 1);
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (occ & 1 << i) {
			drw_setscheme(drw, scheme[SchemeBox]);
